...
public class GraphWeighted {
    ...
    public NodeWeighted getNodeNodeWeighted(String port) {
    	NodeWeighted r = null;
    	for(NodeWeighted aSiteId: nodes) {
    	    r = aSiteId;
    	    if(aSiteId.name.equals(port)) {
    	    	return r;
    	    }
    	}
    	return null;
    ...
    public String DijkstraShortestPath(NodeWeighted start, NodeWeighted end) {
        ...
        while (true) {
            ...
            if (currentNode == null) {
            	//str += "There isn't a path between " + start.name + " and " + end.name + "\n";
                return null;
            }

            // If the closest non-visited node is our destination, we want to print the path
            if (currentNode == end) {
                str += ("The path with the smallest weight between "
                ...
                str += (path) + "\n";
                str += ("The path costs: " + shortestPathMap.get(end)) + "\n";
                return str;
            }
            currentNode.visit();
